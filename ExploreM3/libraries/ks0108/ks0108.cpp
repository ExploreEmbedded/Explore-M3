...
const int lcdPins[8] = {D0,D1,D2,D3,D4,D5,D6,D7};
...
void lcdDataDir(uint8_t value)
{
    ...
    for(i=0;i<8;i++)
    {
       pinMode(lcdPins[i],util_GetBitStatus(value,i)); 
    }
}
...
{ 
   ...
   for(i=0;i<8;i++)
    {
       digitalWrite(lcdPins[i],util_GetBitStatus(value,i)); 
    }
...
void ks0108::WaitReady( uint8_t chip){
	...
	fastWriteLow(EN); 	
//EN_DELAY();EN_DELAY();EN_DELAY();EN_DELAY();
