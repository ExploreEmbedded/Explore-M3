 ...
 * Description: Program to demonstrate the task switching depending on the priorities
				And changing the task priorities on the fly
...
static void MyTask1(void* pvParameters)
{
  ...
  Serial0.printf("\n\rIn Task1 with priority=%d, waiting for some time", uxTaskPriorityGet(TaskHandle_1));

  vTaskDelay(200);

  LED_Glow(LED_Task1);	 /* Led to indicate the execution of Task1*/
  Serial0.printf("\n\rBack in Task1, With priority=%d", uxTaskPriorityGet(TaskHandle_1));
...
static void MyTask2(void* pvParameters)
{
  ...
  Serial0.printf("\n\rIn Task2 with priority=%d, waiting for some time", uxTaskPriorityGet(TaskHandle_2));

  vTaskDelay(200);

  LED_Glow(LED_Task2);	 /* Led to indicate the execution of Task2*/
  Serial0.printf("\n\rBack in Task2, With priority=%d", uxTaskPriorityGet(TaskHandle_2));
...
static void MyTask3(void* pvParameters)
{
  ...
  LED_Glow(LED_Task3);	 /* Led to indicate the execution of Task3*/
  Serial0.printf("\n\rBack in Task3, Changing priority of Task2 and Task1");
  vTaskPrioritySet(TaskHandle_2, 4);
