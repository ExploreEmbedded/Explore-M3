 ...
 */
  ...
  for (uint8_t i = 0; i < 8; ++i)
  {
    digitalWrite(dataPin, (data & 0x80) ? HIGH : LOW);delay_us(10);
    //  mosifast((data & 0x80) ? HIGH : LOW); // faster
   digitalWrite(clockPin, HIGH); delay_us(10);// do not change to high speed!
   digitalWrite(clockPin, LOW); delay_us(10); // do not change to high speed pin change!
