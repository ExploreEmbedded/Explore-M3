...
void PWM_Init(void)
{
    ...
    LPC_PWM1->MCR = 3;    /* interrupt on PWMMR0, reset on PWMMR0, reset 
                                TC if PWM matches */                
    LPC_PWM1->MR0 = 255;        /* set PWM cycle */
...
/***************************************************************************************************
                         void PWM_Set( uint32_t pin, uint32_t dutyCycle )
****************************************************************************************************
...
{  
   ...
   switch(pin)
   {
    ...
    default :
            GPIO_PinFunction(pin,PINSEL_FUNC_0);
            GPIO_PinDirection(pin,OUTPUT);
            if(dutyCycle<128)
            {
                GPIO_PinWrite(pin,LOW);
            }
            else
            {
                GPIO_PinWrite(pin,HIGH);
            }
            break;            
     }
}        
